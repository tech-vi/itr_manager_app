...
const RowActions = ({ data }) => {
  ...
  const [editITRFormStatus] = useEditITRFormStatusMutation();
  const [removeITRFormStatus] = useRemoveITRFormStatusMutation();
  ...
  const onDelete = async () => {
    try {
      setLoading(true);
      const response = await removeITRFormStatus(data._id).unwrap();
  ...
  const onSubmit = async (editedData) => {
    ...
    try {
      const response = await editITRFormStatus({
        id: data._id,
      ...
      // console.error("Error updating itr form status:", error);
    }
  };
  ...
  return (
    <div>
      ...
      <CustomModal
        ...
        onSubmit={onSubmit}
        modalTitle="Edit ITR Form Status"
        modalBody={modalBody}
      ...
      <Dropdown>
        <Dropdown.Toggle
          variant="light"
          id="row-actions-menu" className="row-actions-menu"
